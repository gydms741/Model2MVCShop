...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	<tr>
		...
		<td align="right">
			...
			<option value="0" ${! empty search.searchCondition && search.searchCondition==2 ? "selected" : "" }>상품가격</option>
			</select>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	...
	<tr class="ct_list_pop">
		<td align="center"><%=i+1%></td><!--  No : 3 2 1표시 -사용 줄어든다 -->
		<td></td>
				
				<td align="left">
								
				<% if(menu.equals("manage")){ %>
				<a href="/updateProductView.do?prodNo=<%=product.getProdNo()%>&menu=<%=menu %>">
				<%=product.getProdName() %>
				<%} else { %>
				<a href="/getProduct.do?prodNo=<%=product.getProdNo()%>&menu=<%=menu %>">
				<%=product.getProdName() %>
				<%} %>
...
<c:set var="i" value="0" />
	<c:forEach var="product" items="${list }">
		...
		<tr class="ct_list_pop">
			...
			<a href="/updateProductView.do?prodNo=${product.prodNo}&${param.menu }">${product.prodName}</a>
			</c:when>
			<c:otherwise>
